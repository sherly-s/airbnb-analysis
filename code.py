...
@st.cache_data
def get_data():
  return pd.read_csv("C:/Users/DELL/anaconda3/envs/ytt/proj/listings.csv")

@st.cache_data
def get_profile_pic():
  return Image.open('https://res.cloudinary.com/dyd911kmh/image/upload/v1640050215/image27_frqkzv.png')
...
def main():
  ...
  st.sidebar.image("https://res.cloudinary.com/dyd911kmh/image/upload/v1640050215/image27_frqkzv.png", use_column_width=False, width=250)
  ...
  room_types_df['Percentage'] = room_types_df.groupby(['District'])['Quantity'].apply(lambda x: 100 * x / x.sum()).reset_index(drop=True)
